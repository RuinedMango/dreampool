		...
		for (char ch : text.toCharArray()){
			if (ch >= 32 && ch <= 126){
				if (ch == ' '){
					...
					Vector2f glyphSize = new Vector2f((packedChar.x1() - packedChar.x0()) * scaleFactor, (packedChar.y1() - packedChar.y0()) * scaleFactor);

					Vector2f glyphBoundingBoxBottomLeft = new Vector2f(localPos.x + (packedChar.xoff() * scaleFactor), localPos.y - (packedChar.yoff() + packedChar.y1() - packedChar.y0()) * scaleFactor);

					Vector2f[] glyphVertices = {new Vector2f(glyphBoundingBoxBottomLeft.x + glyphSize.x, glyphBoundingBoxBottomLeft.y + glyphSize.y), new Vector2f(glyphBoundingBoxBottomLeft.x, glyphBoundingBoxBottomLeft.y + glyphSize.y), new Vector2f(glyphBoundingBoxBottomLeft.x, glyphBoundingBoxBottomLeft.y), new Vector2f(glyphBoundingBoxBottomLeft.x + glyphSize.x, glyphBoundingBoxBottomLeft.y),};

					Vector2f[] glyphTextureCoords = {new Vector2f(alignedQuad.s1(), alignedQuad.t0()), new Vector2f(alignedQuad.s0(), alignedQuad.t0()), new Vector2f(alignedQuad.s0(), alignedQuad.t1()), new Vector2f(alignedQuad.s1(), alignedQuad.t1())};
